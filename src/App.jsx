...
function App() {
  ...
  // --- FUNÇÕES DE ADMIN ---
  async function handleAdminLogin(e) { e.preventDefault();
     setAdminError(null); setLoading(true); 
     try { const { data: uD, error: uE } = await supabase.from('user_mestre').select('apelido, senha_hash').eq('apelido', adminApelido).single();
       if (uE && uE.code !== 'PGRST116') throw uE; if (!uD || uE) throw new Error('Inválido.'); const sP = uD.senha_hash;
       if (adminPassword === sP) { setIsMasterAdmin(true); setView('admin_db_select'); } else { throw new Error('Inválido.');
       } } catch (err) { console.error('Erro login:', err); setAdminError(err.message || 'Erro.'); } finally { setLoading(false); } }
  ...
  async function handleViewHistoryDetails(userId, userNickname) { 
    ...
    try { 
      if (!isOld) { 
        console.log("ADMIN DETALHES: Buscando ranking para (Novo DB) ID:", userId); // DEBUG
        ...
        if(rD&&rD.length>0&&rD[0].ranking_completo){
          ...
          try {
            ...
            if (top3.length > 0) {
              ...
              console.log("ADMIN DETALHES: Sugestões calculadas:", suggestions); // DEBUG
              setHistorySuggestions(suggestions);
            } else {
              console.log("ADMIN DETALHES: Top 3 está vazio, sem sugestões."); // DEBUG
  ...
  const renderResult = () => {
    ...
    return (
      <div className="container result-container">
        ...
        {/* Adiciona o 'suggestions-container' para aplicar o estilo de caixa */}
        {finalResult.sugestoes && finalResult.sugestoes.length > 0 && (
          <div className="suggestions-container">
            {/* O título agora tem a classe correta */}
            <h3 className="suggestions-title">Sugestões (Top 7)</h3>
            <div className="suggestions">
              <ul> 
                {finalResult.sugestoes.map((c, i) => ( 
                  <li key={i}>{c}</li> 
                ))} 
              </ul>
            </div>
  ...
  // =================================================================
  // ATUALIZADO: renderHistory (CSS condicional)
  // =================================================================
  const renderHistory = () => ( 
    <div className={`container history-container ${adminSelectedDb === 'old' ? 'history-container-old' : ''}`}>
      ...
      {!historyLoading&&allDbResults.length>0&&( 
        <ul className="result-list">{allDbResults.map((r)=>( 
          <li key={`${r.id_u}-${r.date}-${r.time}`} className="result-item">
            <div><strong>Apelido: </strong><button onClick={()=>handleViewHistoryDetails(r.id_u,r.nickname)} className="history-nickname-button">{r.nickname}</button> (ID: {r.id_u})</div>
            <div><strong>Data:</strong> {r.date} às {r.time}</div>
            <div><strong>Foco:</strong> {r.foco}</div>
          </li> 
        ))}</ul> 
      )}
      {!historyLoading&&allDbResults.length===0&&!adminError&&( <p className="no-results-message">Nenhum resultado.</p> )}
      <div className="extra-buttons">
  ...
  // =================================================================
  // ATUALIZADO: renderDetailView (CSS condicional)
  // =================================================================
  const renderDetailView = () => { 
    if (!detailedUser) { setView('history'); return null; } 
    return ( 
      <div className={`container detail-view-container ${adminSelectedDb === 'old' ? 'detail-view-old' : ''}`}>
  ...
  return (
    <div className="app-container">
      ...
      >
        &copy; {new Date().getFullYear()} por Amandy Uchiha. Todos os direitos reservados.
      </footer>
    </div>
  );
}
